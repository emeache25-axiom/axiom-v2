const MarketScreen = {
  ...
  _change(n) {
    ...
    const sign  = n>0 ? '+' : '';
    return `<span style="font-family:var(--f2);font-size:12px;font-weight:600;
                         color:${color};">${sign}${n.toFixed(2)}%</span>`;
  },
  ...
  _isCacheValid(view) {
    ...
    const elapsed = (Date.now() - this.cacheTime[view]) / 1000 / 60;
    return elapsed < this.cacheTTL[view];
  },
  ...
  _coinsTableRow(c) {
    const spark = this._sparkline(c.sparkline || [], c.change_7d);
    return `
    <div class="coins-row"
         style="display:grid;grid-template-columns:36px minmax(140px,1fr) 90px 65px 65px 80px 90px 90px;
      ...
      <span style="font-family:var(--f2);font-size:10px;color:var(--t3);
                   ...
                   position:sticky;left:0;background:var(--c1);z-index:1;">${c.rank}</span>
      <div style="display:flex;align-items:center;gap:6px;min-width:0;
                  position:sticky;left:36px;background:var(--c1);z-index:1;">
        ${this._avatar(c.symbol, c.image)}
        <div style="min-width:0;">
          <div style="font-weight:500;color:var(--t1);font-size:12px;
                      white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${c.name}</div>
          <div style="font-family:var(--f2);font-size:9px;color:var(--t3);">${c.symbol}</div>
        </div>
      </div>
      <span style="font-family:var(--f2);font-size:11px;color:var(--t1);
                   text-align:right;">${this._price(c.price)}</span>
      <span style="text-align:right;">${this._change(c.change_24h)}</span>
      <span style="text-align:right;">${this._change(c.change_7d)}</span>
      <div style="display:flex;justify-content:center;">${spark}</div>
      <span style="font-family:var(--f2);font-size:11px;color:var(--t3);
                   text-align:right;">${this._fmt(c.market_cap)}</span>
      <span style="font-family:var(--f2);font-size:11px;color:var(--t3);
  ...
  _renderGeneral(data) {
    ...
    <div class="card" style="border-top:2px solid #B47514;border-left:1px solid #B4751440;
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;
        ...
        <span id="coins-page-info"
              style="font-family:var(--f2);font-size:11px;color:var(--t3);">Página 1</span>
  ...
  async _loadDrilldown(supercatId) {
    ...
    try {
      ...
      const el2 = document.getElementById('market-content');
      if (el2) el2.innerHTML = `<div class="placeholder"><i class="ti ti-alert-circle"></i>
        <p>Error al cargar</p></div>`;
    }
  },
  ...
  _renderDrilldown(data, supercatId) {
    ...
    <div style="display:grid;grid-template-columns:36px minmax(140px,1fr) 90px 65px 65px 80px 90px 90px;
      ...
      <span style="font-family:var(--f2);font-size:10px;color:var(--t3);
                   text-align:right;padding-right:4px;
                   position:sticky;left:0;background:var(--c1);z-index:1;">
        ${c.rank || i+1}
      </span>
      <div style="display:flex;align-items:center;gap:6px;min-width:0;
                  position:sticky;left:36px;background:var(--c1);z-index:1;">
      ...
      <div style="display:flex;justify-content:center;">
        ${this._sparkline(c.sparkline || [], c.change_7d)}
      </div>
      <span style="font-family:var(--f2);font-size:11px;color:var(--t3);
                   text-align:right;">${this._fmt(c.market_cap)}</span>
      <span style="font-family:var(--f2);font-size:11px;color:var(--t3);
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;
      ...
      <div style="display:flex;align-items:center;gap:10px;">
        <button onclick="MarketScreen.switchView('categories')"
          ...
          onmouseover="this.style.color='#F5F0EB'"
          onmouseout="this.style.color='var(--t3)'">
          <i class="ti ti-chevron-left" style="font-size:13px;"></i> Categorías
        </button>
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:10px;height:10px;border-radius:50%;
                      background:${sc.color};flex-shrink:0;"></div>
    ...
    <div class="card" style="border-top:2px solid ${sc.color};
                              ...
                              border-bottom:1px solid ${sc.color}40;">
      <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">
        <div style="min-width:620px;">
          <div style="display:grid;
                      grid-template-columns:36px minmax(140px,1fr) 90px 65px 65px 80px 90px 90px;
                      gap:6px;font-family:var(--f2);font-size:9px;color:var(--t3);
                      text-transform:uppercase;letter-spacing:.08em;
                      padding-bottom:6px;border-bottom:1px solid var(--w1);">
            <span style="text-align:right;position:sticky;left:0;background:var(--c1);z-index:1;">#</span>
          ...
          <div style="display:flex;align-items:center;gap:6px;min-width:0;">
            ...
            <div style="position:relative;display:inline-flex;" class="info-wrap">
              ...
              <div class="info-tooltip"
                   style="display:none;position:absolute;left:0;top:24px;
                          z-index:200;width:min(240px,calc(100vw - 40px));
                          padding:10px 12px;background:#1A1917;
                          border:0.5px solid ${c.color};border-radius:8px;
                          font-size:11px;color:var(--t2);line-height:1.5;
                          box-shadow:0 4px 16px rgba(0,0,0,.4);">
                <div style="font-size:12px;font-weight:600;color:#F5F0EB;
                            margin-bottom:5px;">${c.label}</div>
                ${c.info}
              </div>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:12px;flex-shrink:0;">
            <span style="font-family:var(--f2);font-size:11px;color:${cc};">
              ${sign}${c.change_24h.toFixed(1)}%</span>
            <span style="font-family:var(--f2);font-size:11px;color:var(--t3);">${c.pct}%</span>
            <span style="font-family:var(--f2);font-size:11px;color:var(--t3);">
              ${this._fmt(c.mcap)}</span>
          </div>
        </div>
        <div style="height:5px;background:var(--c3);border-radius:3px;">
          <div style="height:5px;width:${(c.pct/max*100).toFixed(1)}%;
                      background:${c.color};border-radius:3px;transition:width .4s;"></div>
        </div>
      </div>`;
    }).join('');
  ...
  _renderNetworks(data) {
    ...
    <div style="margin-bottom:16px;">
      <div style="display:flex;align-items:center;justify-content:space-between;
        ...
        <div style="display:flex;align-items:center;gap:6px;min-width:0;">
          <div style="width:10px;height:10px;border-radius:50%;
                      background:${n.color};flex-shrink:0;"></div>
          <span style="font-size:13px;font-weight:500;color:#F5F0EB;">${n.label}</span>
          <div style="position:relative;display:inline-flex;" class="info-wrap">
            <i class="ti ti-info-circle"
               style="font-size:13px;color:var(--t3);cursor:pointer;flex-shrink:0;"
               aria-hidden="true"></i>
            <div class="info-tooltip"
                 style="display:none;position:absolute;left:0;top:24px;
                        z-index:200;width:min(240px,calc(100vw - 40px));
                        padding:10px 12px;background:#1A1917;
                        border:0.5px solid ${n.color};border-radius:8px;
                        font-size:11px;color:var(--t2);line-height:1.5;
                        box-shadow:0 4px 16px rgba(0,0,0,.4);">
              <div style="font-size:12px;font-weight:600;color:#F5F0EB;
                          margin-bottom:5px;">${n.label}</div>
              ${n.info}
            </div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:12px;flex-shrink:0;">
          <span style="font-family:var(--f2);font-size:11px;color:var(--t3);">${n.pct}%</span>
          <span style="font-family:var(--f2);font-size:11px;color:var(--t3);">
            ${this._fmt(n.mcap)}</span>
        </div>
      </div>
      <div style="height:5px;background:var(--c3);border-radius:3px;">
        <div style="height:5px;width:${(n.pct/max*100).toFixed(1)}%;
                    background:${n.color};border-radius:3px;transition:width .4s;"></div>
      </div>
    </div>`).join('');

    return this._refreshBtn('networks') + `
    <div class="card" style="border-top:2px solid #627EEA;border-left:1px solid #627EEA40;
                              border-right:1px solid #627EEA40;border-bottom:1px solid #627EEA40;">
      ${this._sectionHeader('ti-topology-star-3','Distribución por red','#627EEA')}
      ${rows}
    </div>`;
  },
